...
FramelessDialog::FramelessDialog(QWidget *parent) : QDialog(parent)
{
    ...
    if (nativeResize == false){
        ...
        if (shadowEnable){
            drawShadow = new DrawShadowHelper(this);
...
void FramelessDialog::doResizeEvent(QEvent *event)
{
    ...
    if (event->type() == QEvent::Resize) {
            ...
            windowState() == Qt::WindowMaximized)
        {
            ...
            float fx = (float)point.x() / (float)this->rect().width();
            int offsetY = point.y();
            emit titleDblClick();
            ...
            this->move(x1, y1);
            //重新计算移动起点和大小
            mousePoint = this->mapFromGlobal(point);
        ...
        if (pressedRect.at(0).contains(mousePoint)) {
            ...
            titleRect = QRect(titleBar->mapTo(this, titleRect.topLeft()), titleRect.size());
